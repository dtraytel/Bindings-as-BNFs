...
imports More_Datatype_Bindings
begin
...
type_synonym 'a P = "'a ssfun"
type_synonym 'a C = "'a TT"

definition A :: "'a::var_TT set" where "A = {}"
definition CCTOR :: "('a::var_TT, 'a, 'a TT \<times> ('a P \<Rightarrow> 'a C), 'a TT \<times> ('a P \<Rightarrow> 'a C)) F \<Rightarrow> 'a P \<Rightarrow> 'a C" where
  "CCTOR = (\<lambda>F p. cctor (map_F (Rep_ssfun p) id ((\<lambda>R. R p) o snd) ((\<lambda>R. R p) o snd) F))"
definition mmapC  :: "('a::var_TT \<Rightarrow> 'a) \<Rightarrow> 'a TT \<Rightarrow> 'a C \<Rightarrow> 'a C" where
  "mmapC = (\<lambda>f _ x. map_TT f x)"
definition FFVarsC  :: "'a TT \<Rightarrow> 'a::var_TT C \<Rightarrow> 'a set" where
  "FFVarsC = (\<lambda>_ x. FFVars x)"
definition mapP  :: "('a::var_TT \<Rightarrow> 'a) \<Rightarrow> 'a P \<Rightarrow> 'a P" where
  "mapP = (\<lambda>u p. compSS u p)"
definition FVarsP  :: "'a::var_TT P \<Rightarrow> 'a set" where
  "FVarsP = (\<lambda>p. imsupp (Rep_ssfun p))"

lemmas model_defs = A_def CCTOR_def mmapC_def FFVarsC_def mapP_def FVarsP_def
...
lemma
  ...
  (*  *)
  and
  (* Next are the two model clauses from the paper, more precisely from the Section A.2 (from appendix.pdf) which
...
definition FFVarsBC where "FFVarsBC X p \<equiv> \<Union> (t,pd) \<in> set4_F X. (FFVars t \<union> FFVarsC t (pd p)) - set2_F X"

abbreviation mmapPC where "mmapPC \<equiv> mmapfn mapP mmapC"
...
abbreviation mapPC where "mapPC \<equiv> mapfn mapP mapC"
...
lemma vvsubst_cctor: "set2_F x \<inter> (imsupp f) = {} \<Longrightarrow> nnoclash x \<Longrightarrow>
  ...
  by (subst ff0_cctor[unfolded model_defs])
...
lemma FFVars_vvsubst_weak: "FFVars (vvsubst t) \<subseteq> FFVars t \<union> imsupp f"
  unfolding vvsubst_def
  by (rule order_trans[OF ff0_FFVars[unfolded model_defs]])
    (auto simp: F_map_comp[symmetric] f supp_id_bound o_def id_def[symmetric] Rep_ssfun fSS.rep_eq)
